...
CALIBRATION_CASES = ROOT / "benchmarks" / "assistance" / "calibration_cases_a44c.py"
...
def test_a44c_scope_is_calibration_only() -> None:
    ...
    assert scope["validation_case_rows_materialized_authorized"] == 0
...
def test_a44c_gold_pair_arithmetic_is_registered() -> None:
    ...
    assert sum(int(value) for value in counts.values()) == 491


def test_a44c_artifact_contract_forbids_validation_materialization() -> None:
    config = _config()
    contract = config["artifact_contract"]
    assert isinstance(contract, dict)
    assert contract["validation_cases_must_not_be_materialized"] is True
    assert contract["validation_rows_must_not_be_written"] is True
    assert contract["validation_metrics_must_not_be_computed"] is True


def test_a44c_execution_files_parse_without_running_models() -> None:
    ast.parse(CALIBRATION_CASES.read_text(encoding="utf-8"))
    ...
    assert "generate_calibration_cases" in execution_source
    assert "generate_cases()" not in execution_source
...
def test_a44c_workflow_is_push_to_main_one_shot_path_scoped() -> None:
    ...
    assert "calibration_cases_a44c.py" in workflow
